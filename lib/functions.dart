...
import 'package:med_reminder/pages/edit_medicine.dart';
import 'package:med_reminder/pages/scan/scanpage.dart';
...
String dosagef =
    r'.(?: (morn|mom|morm|nigh|night|noon|eve)(\w+)?(?: \w+)?)|\d+ (tab|cap)';
String timef =
    r'(?:once|twice|\d+ times) (?:a|every) (\w+(?: \w+)*)|(?:(before|after) food)';
...
List<Medicine> parseMedicines(String text) {
  ...
  int startIndex = lines.indexWhere((note) =>
      note.toLowerCase().startsWith('medicine') ||
      note.toLowerCase().startsWith('dosage'));
  if (startIndex == -1) {
    startIndex = 0;
  }
  ;
  ...
  for (int i = startIndex; i < lines.length; i++) {
    Iterable<RegExpMatch> medicationNameMatches =
        medicationNamePattern.allMatches(lines[i]);
    Iterable<RegExpMatch> dosageMatches = dosagePattern.allMatches(lines[i]);
    Iterable<RegExpMatch> frequencyMatches =
        frequencyPattern.allMatches(lines[i]);
    Iterable<RegExpMatch> durationMatches =
        durationPattern.allMatches(lines[i]);
    Iterable<RegExpMatch> unwantMatches = unwantPattern.allMatches(lines[i]);

    print(lines[i]);
    // print(medicationNameMatches);
    try {
      if (medicationNameMatches.isNotEmpty &&
          lines[i].length < 30 &&
          durationMatches.isEmpty &&
          dosageMatches.isEmpty &&
          unwantMatches.isEmpty &&
          lines[i].toLowerCase() != 'capsule' &&
          lines[i].toLowerCase() != 'tablet' &&
          lines[i].startsWith(RegExp(r'\d+'))) {
        //print('mn $mn');
        //print(lines[i]);
        if (mn >= md && mn >= mf && mn >= mdu) {
          // print(2);
          scanmedicines.add(Medicine(
              name: _setname(lines[i]),
              dosage: '',
              time: '',
              duration: '',
              startdate: null));
...
String? _settime(String dosage, String time) {
  ...
  print(time);
  String set = '';
  if (dosage.toLowerCase().contains('mo')) {
    set += 'Morning ';
  }
  if (dosage.toLowerCase().contains('aft')) {
    set += 'Afternoon ';
  }
  if (dosage.toLowerCase().contains('nig')) {
    set += 'Night ';
  }
  if (time.toLowerCase().contains('bef') ||
      time.toLowerCase().contains('ter fo')) {
    set += '\n';
  }
  if (time.toLowerCase().contains('bef')) {
    set += 'BeforeFood ';
  }
  if (time.toLowerCase().contains('ter fo')) {
    set += 'AfterFood';
  }
...
Future<void> scheduleNotification(
  ...
  try {
    int id =
        '${title}_${dateTime.month}_${dateTime.day}_${dateTime.hour}'.hashCode;
...
Future<void> scheduleMedicineNotification(Medicine medicine,
    {Medicine? oldMedicine}) async {
  if (oldMedicine != null) {
    await cancelOldNotifications(oldMedicine);
  }

  DateTime? startDate = medicine.startdate;
  int days = medicine.duration == 'everyday'
      ? 30
      : int.parse(medicine.duration!.split(' ')[0]);
  print('schdule med ${medicine.name}');
  int c = 0;
  for (int i = 0; i < days; i++) {
    if (medicine.time!.contains('Morning')) {
      DateTime notificationDateTime = DateTime(startDate!.year, startDate.month,
          startDate.day + i, morningTime.hour, morningTime.minute);

      String? food = medicine.time!.contains('food')
          ? medicine.time!.split('\n')[1]
          : null;
      String ftext = food == null ? '' : 'at $food';

      await scheduleNotification(medicine.name!,
          'Take ${medicine.dosage} $ftext', notificationDateTime);
      c++;
      print('No $c');
    }
    if (medicine.time!.contains('Afternoon')) {
      DateTime notificationDateTime = DateTime(startDate!.year, startDate.month,
          startDate.day + i, afternoonTime.hour, afternoonTime.minute);
      String? food = medicine.time!.contains('food')
          ? medicine.time!.split('\n')[1]
          : null;
      String ftext = food == null ? '' : 'at $food';
    ...
    if (medicine.time!.contains('Night')) {
      DateTime notificationDateTime = DateTime(startDate!.year, startDate.month,
          startDate.day + i, nightTime.hour, nightTime.minute);
      String? food = medicine.time!.contains('food')
          ? medicine.time!.split('\n')[1]
          : null;
      String ftext = food == null ? '' : 'at $food';
...
Future<void> cancelOldNotifications(Medicine oldMedicine) async {
  try {
    ...
    int days = oldMedicine.duration == 'everyday'
        ? 30
        : int.parse(oldMedicine.duration!.split(' ')[0]);
    int c = 0;
    for (int i = 0; i < days; i++) {
      if (oldMedicine.time!.contains('Morning')) {
        DateTime dateT = DateTime(startDate!.year, startDate.month,
            startDate.day + i, morningTime.hour, morningTime.minute);
        int id = '${oldMedicine.name}_${dateT.month}_${dateT.day}_${dateT.hour}'
            .hashCode;
        print(id);
...
Future<void> saveTimes() async {
  ...
  // Save nightTime
  await prefs.setString('nightTime', '${nightTime.hour}:${nightTime.minute}');
...
Future<void> scheduleMedicineAlarm(Medicine medicine,
    {Medicine? oldMedicine}) async {
  if (oldMedicine != null) {
  ...
  for (int i = 0; i < days; i++) {
    if (medicine.time!.contains('Morning')) {
      ...
      ScheduleAlarm(
          medicine, 'Take ${medicine.dosage} $ftext', notificationDateTime);
      c++;
      print('No $c');
    }
