...
String _settime() {
  ...
  if (_afternoonChecked) {
    text += 'Afternoon ';
  }
  if (_nightChecked) {
    text += 'Night ';
  }
  if (_beforefood || _afterfood) {
    text += '\n';
  }
  if (_beforefood) {
    text += 'BeforeFood ';
...
String durationPattern =
    r'\d+'; // Regular expression pattern to match the number of days
            ...
            children: <Widget>[
              Container(
                ...
                decoration: BoxDecoration(
                  color: Color.fromARGB(255, 238, 252,
                      223), // Setting background color to light green
                  borderRadius:
                      BorderRadius.circular(10), // Rounding the corners
                ),
                child: Column(
                  children: [
                    TextField(
                  ...
                  child: ElevatedButton(
                      ...
                      onPressed: () async {
                        ...
                        if (_nameController.text == "") {
                          displayError('Enter the Medicine Name');
                          return;
                        } else if (_dosageController.text == "") {
                          displayError('Enter Dosage value');
                          return;
                        } else if (!_morningChecked &&
                            !_afternoonChecked &&
                            !_nightChecked) {
                          displayError('Select Time');
                          return;
                        }
                        //else if(!_beforefood&&!_afterfood){displayError('Select Before food/After food');return;}
                        else if (!_isEveryDay && _noofdays.text == "") {
                          displayError('Enter No. of Days');
                          return;
                        }
                        ...
                        await scheduleMedicineNotification(med,
                            oldMedicine: oldMed);
                      }))
            ],
          ),
